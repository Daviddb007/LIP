"""API REST pública v1 del ecosistema."""
...
def requerir_role(role_minimo: str):
    from functools import wraps
...
def api_participaciones():
    ...
    if sector:
        from app.models.catalog import participacion_problemas, ProblemaCatalogo, Subsector
        query = query.join(participacion_problemas).join(
            ProblemaCatalogo, participacion_problemas.c.problema_id == ProblemaCatalogo.id
        ).join(Subsector).join(Sector).filter(Sector.nombre == sector)
    if departamento:
...
def api_sectores():
    ...
    return jsonify({"total": len(sectores), "data": [s.to_dict() for s in sectores]})
